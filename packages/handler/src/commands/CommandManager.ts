...
export class CommandManager extends BaseManager {
  private _commands: Collection<string, Commands> = new Collection<
    string,
    Commands
  >();
  ...
  private async loadCommand(folderPath: string): Promise<void> {
    ...
    for await (const folderName of topLevelFolders) {
      switch (folderName) {
        ...
        default: {
          this.client.logger.info(
            `Command Handler: -> Unknown folder -> ${folderName}`,
          );
          break;
        }
      }
  ...
  private async loadCommands(folderPath: string): Promise<boolean> {
    for await (const file of findFilesRecursively(
      ...
      const cmd_ = container.resolve<CommandInterface>(
  ...
  private async RegisterApplicationCommands() {
    // Global commands
    const globalCommands = Array.from(this._commands.values()).filter(
      (cmd) => cmd.guildId === null,
    );
    ...
    const guildIdsArray = Array.from(guildCommands.values())
      ...
      .filter(
        (guildId, index, array) =>
          guildId !== null && array.indexOf(guildId) === index,
      );
  ...
  private async HandleApplicationCommands(i: CommandInteraction) {
    ...
    try {
      ...
      const cmd = container.resolve<
        MessageCommandInterface | SlashCommandInterface | UserCommandInterface
