	...
	public override async execute(interaction: ChatInputCommandInteraction) {
		...
		if (durationStr) {
			...
			if (durationSeconds <= 0) {
				await interaction.editReply({ content: "Invalid duration (e.g. 1d, 2w, 30d)." }).catch(() => null)
				return
			}
		...
		try {
			...
			this.container.client.logger.error(new Error(`Failed to ban user ${userId}: ${msg}`))
			if (msg.includes("Unknown interaction")) return
			await interaction
				.editReply({ content: "❌ An error occurred while banning the user. Please try again." })
				.catch(() => null)
		}
		return
	}
