...
export default class ConfigCommand implements SlashCommandInterface {
  ...
  public async execute(
    ...
    if (channelConfig) {
        ...
        for (const values of subscribedLogsSelectOptions[channelConfig.type]) {
          options.push({
            ...
            default: channelConfig[
              values.value as keyof typeof channelConfig
            ] as boolean,
          });
        }
      ...
      await interaction.channel
        .awaitMessageComponent({
          ...
          filter: (i) =>
            ...
            i.customId.startsWith("configChannel:config:"),
        })
        .then(async (i) => {
          await i.deferUpdate();
          switch (i.customId) {
            case "configChannel:config:subSelect": {
              ...
              const typeValue = subscribedLogsSelectOptions[
                channelConfig.type
              ].map((v) => v.value);
      ...
      await interaction.reply({
        content:
          "This channel is not yet configured. Do you want to configure it?",
        ephemeral: true,
        components: [
          {
            type: ComponentType.ActionRow,
      ...
      await interaction.channel
        .awaitMessageComponent({
          ...
          filter: (i) =>
            i.user.id === interaction.user.id &&
            i.customId.startsWith("configChannel:newChannel:new") &&
            i.isButton(),
        })
        .then(async (i) => {
          await i.deferUpdate();
          if (i.customId === "configChannel:newChannel:yes") {
            ...
            await i.channel
              .awaitMessageComponent({
                ...
                filter: (i) =>
                  ...
                  i.customId.startsWith(
                    "configChannel:newChannel:typeSelect",
                  ) &&
              ...
              .then(async (i) => {
                ...
                if (!i.isStringSelectMenu()) return;
                const type = i
                  .values[0] as keyof typeof subscribedLogsSelectOptions;
                const webhookInfo: {
                ...
                if (channel.isThread()) {
                  webhook = await channel.parent.createWebhook({
                    ...
                    reason:
                      "New log channel created by " + interaction.user.tag,
                  });
