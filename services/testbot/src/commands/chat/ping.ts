...
export default class PingCommand implements SlashCommandInterface {
	...
	public async execute(interaction: ChatInputCommandInteraction<`cached`>, client: HandlerClient) {
		try {
			interaction.reply({
				content: 'Pong!',
				ephemeral: true,
	...
	public async autocomplete(interaction: AutocompleteInteraction, client: HandlerClient) {
		try {
			interaction.respond([
				{
					name: 'Pong!',
