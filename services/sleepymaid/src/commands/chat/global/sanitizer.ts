    ...
    if (interaction.options.getSubcommand() === "toggle") {
      ...
      if (interaction.options.getSubcommand() === "add") {
        ...
        await client.drizzle
          ...
          .set({
            sanitizerIgnoredRoles: [
              ...guildSettings!.sanitizerIgnoredRoles!,
              role.id,
            ],
          })
          .where(eq(guildsSettings.guildId, interaction.guildId));
        ...
        await client.drizzle
          .update(guildsSettings)
          .set({
            sanitizerIgnoredRoles: guildSettings!.sanitizerIgnoredRoles!.filter(
              (r) => r !== role.id,
            ),
          })
          .where(eq(guildsSettings.guildId, interaction.guildId));
        ...
        await client.drizzle
          ...
          .set({
            sanitizerIgnoredRoles: [
              ...new Set(
                guildSettings!.sanitizerIgnoredRoles!.filter(
                  (r) => !deletedRoles.includes(r),
                ),
              ),
            ],
          })
