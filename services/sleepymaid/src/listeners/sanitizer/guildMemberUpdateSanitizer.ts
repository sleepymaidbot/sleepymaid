  ...
  public async execute(
    ...
    const sanitizerSettings = (
      await client.drizzle
        .select()
        .from(guildsSettings)
        .where(eq(guildsSettings.guildId, newMember.guild.id))
    )[0]!;
    ...
    if (sanitizerSettings.sanitizerIgnoredRoles!.length) {
      if (
        sanitizerSettings.sanitizerIgnoredRoles!.some((role: string) =>
    ...
    if (
      ...
      if (newMember.nickname !== sanitized)
        await newMember.setNickname(sanitized, "Sanitizer");
