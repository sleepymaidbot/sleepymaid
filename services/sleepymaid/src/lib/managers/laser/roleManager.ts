...
export class laserRoleManager extends baseManager {
	public async startMenu(interaction: ButtonInteraction) {
		...
		collectors.set(
			interaction.user.id,
			interaction.channel!.createMessageComponentCollector<ComponentType.Button | ComponentType.StringSelect>({
		...
		collectors.get(interaction.user.id)!.on('end', (_, reason: EndReason) => {
			if (reason === EndReason.overlap) {
				...
				interaction.editReply({
					content: "You have closed this menu. You can click 'Dismiss message' to dismiss this message.",
					embeds: [],
					components: [],
				});
			} else {
				interaction.editReply({
					content: "The menu has expired. You can click 'Dismiss message' to dismiss this message.",
					embeds: [],
					components: [],
				});
			}
