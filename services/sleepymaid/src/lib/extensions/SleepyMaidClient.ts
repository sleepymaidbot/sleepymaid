...
export class SleepyMaidClient extends HandlerClient {
  ...
  public async start(): Promise<void> {
    ...
    if (mqConnection.rabbitMQConnected)
    ...
    await i18next.use(FsBackend).init({
      ...
      backend: {
        loadPath: resolve(
          __dirname,
          "../../../../../locales/sleepymaid/{{lng}}/{{ns}}.json",
        ),
      },
      cleanCode: true,
      fallbackLng: "en-US",
  ...
  private async startRPCListeners(): Promise<void> {
    await this.channel.assertQueue(Queue.CheckGuildInformation, {
      durable: false,
    });
    void this.channel.consume(Queue.CheckGuildInformation, async (msg) => {
      ...
      const message: CheckGuildInformationRequestMessage = JSON.parse(
      ...
      if (!member || !guild || !this.user) {
        return this.channel.sendToQueue(
          ...
          Buffer.from(JSON.stringify(baseResponse)),
          {
      ...
      if (
        ...
        member.permissions.any(PermissionFlagsBits.ManageGuild, true)
      ) {
        hasPermission = true;
      } else {
        return this.channel.sendToQueue(
