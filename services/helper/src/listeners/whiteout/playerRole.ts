	...
	public async execute(oldMember: GuildMember, newMember: GuildMember): Promise<void> {
		...
		if (newMember.roles.cache.has(playerRole.id)) {
			...
			newMember.roles.cache.forEach((role) => {
				if (role.name.startsWith('3s Whiteout') && role.name.endsWith('Player')) isTeamMember = true;
				else if (role.name.startsWith('2s Whiteout') && role.name.endsWith('Player')) isTeamMember = true;
			});
