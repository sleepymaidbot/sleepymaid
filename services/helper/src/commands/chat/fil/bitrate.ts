  ...
  public async execute(
    ...
    if (interaction.options.getSubcommand() === "now") {
      ...
      if (target.members.size === 0)
        return interaction.reply("There are no members in the voice channel.");
      const channelSettings =
        await client.drizzle.query.randomBitrate.findFirst({
          where: and(
            eq(randomBitrate.guildId, interaction.guild.id),
            eq(randomBitrate.channelId, target.id),
          ),
        });
      if (!channelSettings) {
        if (state === true || !state) {
          ...
          await interaction.reply(
            `The random bitrate has been enabled for ${target.toString()}.`,
          );
        } else {
          await interaction.reply(
            `The random bitrate is already disabled for ${target.toString()}.`,
          );
        }
      } else {
        if (state === true) {
          ...
          await interaction.reply(
            `The random bitrate has been disabled for ${target.toString()}.`,
          );
