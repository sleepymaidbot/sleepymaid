  ...
  public override async execute(
    ...
    if (canFail[interaction.guild.id] && role.color !== 0) {
      if (Math.random() < canFail[interaction.guild.id]!) {
        await role
          .setColors({ primaryColor: 0 }, 'Failed by: ' + interaction.user.tag)
          .then(() => {
    ...
    if (
      !interaction.appPermissions.has(PermissionsBitField.Flags.ManageRoles)
    ) {
      return interaction.editReply({
        embeds: [
          {
    ...
    const hasEnhancedColors = interaction.guild.features.includes(
    ...
    await role
      .setColors(color, `Changed by: ${interaction.user.tag}`)
      .then(() => {
        cooldowns[interaction.guild.id] = add(new Date(), { minutes: 5 });
      });

    const oldEnhanced = oldColor.secondaryColor != null;
    const newHolographic =
      color.primaryColor === Constants.HolographicStyle.Primary;
    ...
    const description = i18next.t(messageKey, {
      ...baseVars,
      oldColor2:
        oldColor.secondaryColor != null
          ? intToHexColor(resolveColor(oldColor.secondaryColor))
          : '',
      newColor2:
        color.secondaryColor != null
          ? intToHexColor(resolveColor(color.secondaryColor))
          : '',
    });
